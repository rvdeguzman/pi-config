...
type Cache = Record<
  string,
  { at: number; wins: Win[]; version?: number; backoffUntil?: number }
>;
...
try {
  showReset =
    (JSON.parse(readFileSync(RESET_PATH, "utf8")) as { show?: boolean })
      .show !== false;
} catch {}
...
async function cached(
  ...
  if (
    !force &&
    e &&
    ((e.version === CACHE_VERSION && now - e.at < TTL[key]) ||
      (e.backoffUntil ?? 0) > now)
  )
    return e.wins;
  try {
  ...
  if (u.five_hour)
    wins.push({
      ...
      resetAt: u.five_hour.resets_at
        ? Date.parse(u.five_hour.resets_at)
        : undefined,
    });
  if (u.seven_day)
    wins.push({
      ...
      resetAt: u.seven_day.resets_at
        ? Date.parse(u.seven_day.resets_at)
        : undefined,
    });
  return wins;
}
...
async function kimiWindows(): Promise<Win[]> {
  ...
  const fiveH =
    u?.limits?.find((l: any) => l?.window?.duration === 300)?.detail ??
    u?.limits?.[0]?.detail;
    ...
    const resetAt =
      typeof w.reset_at === "number"
        ...
        : typeof w.reset_after_seconds === "number"
          ...
          : undefined;
    wins.push({
      label: codexLabel(w.limit_window_seconds),
...
export default function (pi: ExtensionAPI) {
  ...
  function renderQuota(ctx: ExtensionContext) {
    ...
    withLiveCtx(() => {
      ...
      const pct = (n: number) => {
        ...
        return theme.fg(
          v >= 90 ? "error" : v >= 70 ? "warning" : "success",
          `${v}%`,
        );
      };
      ctx.ui.setStatus(
  ...
  pi.registerCommand("quota", {
    description:
      "Force-reload provider quota now (bypasses the cache TTL and 429 backoff)",
    handler: async (_args, ctx) => {
      await refresh(ctx, true, true);
      ctx.ui.notify?.(
        lastWins.length
          ? `Quota: ${formatQuota(lastWins, Date.now(), true)}`
          : "Quota unavailable for the current provider",
        "info",
      );
    },
  });
  ...
  pi.registerCommand("reset", {
    ...
    handler: async (_args, ctx) => {
      ...
      try {
        writeFileSync(
          RESET_PATH,
          `${JSON.stringify({ show: showReset })}\n`,
          "utf8",
        );
      } catch (error) {
        ctx.ui.notify?.(
          `Could not save reset display: ${String(error)}`,
          "error",
        );
      }
      renderQuota(ctx);
      ctx.ui.notify?.(
        `Quota reset countdown ${showReset ? "ON" : "OFF"}`,
        "info",
      );
    },
  });
