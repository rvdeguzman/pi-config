...
> Personal fork for experimentation and hacking. Original authors, attribution, and license are retained.
...
**Model fallback chains.** `model:` accepts a list — a YAML array (`model: [anthropic/claude-opus-4-6, openai/gpt-5]`) or a comma-separated string. Every available candidate is retained in configured order and aliases resolving to the same `provider/modelId` are deduplicated. The first available model starts the run; if it later fails at runtime, the same `AgentSession` advances through the remaining chain. Runtime failover covers thrown provider/HTTP errors (including 429), stream and authentication failures, model initialization/switch failures, terminal `stopReason: "error"`, and an empty output-limit (`stopReason: "length"`) response. User cancellation, an explicit stop, and a hard turn/time deadline never trigger failover.

A failover does not replay the task or recreate the session. Completed messages, tool results, edits, side effects, usage accounting, turn/deadline state, worktree, transcript, and session identity remain intact; the next model receives a short internal handoff telling it to continue from existing state and not repeat completed tool calls. Only a prompt rejected during preflight—before it entered history or could execute tools—is safely submitted unchanged to the next model. Each transition emits `subagents:model_failover`; if every candidate fails, the final error lists every attempted model and failure. This behavior is shared by top-level, nested, scheduled, RPC, resumed, foreground, and detached runs.

When no candidate resolves during initial selection, the agent inherits the parent model exactly as a single unresolvable pin does, so a config chain never hard-fails a spawn. `/agents → Agent types` labels a chain as `primary +N fallback`, and appends `(→ provider/id)` when initial selection lands on something other than the primary. A caller-supplied `model` parameter accepts the same syntax; there it errors instead of inheriting, like any unresolvable caller-supplied model.

**Forgiving `model:` resolution.** Each candidate is matched against pi's model registry tolerantly, so cosmetic id variations don't silently drop the agent back to the parent's model: `.` and `-` are treated as equivalent in version numbers (`claude-haiku-4.5` ≡ `claude-haiku-4-5`), a trailing `-YYYYMMDD` date stamp is optional (`anthropic/claude-haiku-4-5-20251001` matches an undated registry id and vice-versa), and a `provider/modelId` whose named provider doesn't carry that model retries the bare id against every provider. Precedence is **exact → fuzzy under the named provider → same model under any provider → unavailable**, so an exact match always wins and dated snapshots aren't conflated. If nothing resolves, the pin can't run and the agent inherits the parent model — `/agents → Agent types` flags this case as `(unavailable, fallback: inherit)` and shows the resolved target `(→ provider/id)` when resolution lands on a different provider or version than configured. (This is distinct from [Model Scope](#model-scope) enforcement, which matches the `enabledModels` allowlist by *exact* entry.)
...
| `model` | string | no | Model — `provider/modelId` or fuzzy name (`"haiku"`, `"sonnet"`), or a comma-separated fallback chain. Resolved tolerantly (`.`/`-` and a trailing date stamp interchangeable) with provider fallback |
...
- **Agent types** — unified list with source indicators: `•` (project), `◦` (global), `✕` (disabled). Each row shows the agent's model (a fallback chain as `primary +N fallback`), and the highlighted agent's full description appears below the list. The model column flags `(unavailable, fallback: inherit)` when no configured model can be resolved (it would silently inherit the parent model), and shows `(→ provider/id)` when it resolves to a different provider or version than the primary. Select an agent to edit, disable/enable, or delete its backing file.
...
| `subagents:model_failover` | A runtime attempt advances to the next configured model | `id`, `type`, `from`, `to`, `error` |
...
`options.model` accepts either a `Model` object (e.g. `ctx.model`) or a model string. Strings may contain the same comma-separated runtime fallback chain as the `Agent` tool and are resolved against `ctx.modelRegistry` at the RPC boundary, so cross-extension callers can forward serializable values without losing auth context.
