...
**Model fallback chains.** `model:` accepts a list — a YAML array (`model: [anthropic/claude-opus-4-6, openai/gpt-5]`) or a comma-separated string — whose candidates are tried left to right; the first that resolves *and* has auth configured runs. When no candidate resolves the agent inherits the parent model, exactly as a single unresolvable pin does, so a chain never hard-fails a spawn. `/agents → Agent types` labels a chain as `primary +N fallback`, and appends `(→ provider/id)` when the run landed on something other than the primary. A caller-supplied `model` parameter accepts the same syntax; there it errors instead of inheriting, like any unresolvable caller-supplied model.
