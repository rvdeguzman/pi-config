...
describe("input that is not a mention", () => {
  ...
  it("does not retain the old agent- alias", async () => {
    const { lifecycle } = boot();

    expect(await send(lifecycle, "&agent-explore check the RPC path")).toEqual({ action: "continue" });
  });
  ...
  it("leaves every @ reference to pi, including agent-looking ones", async () => {
    const { tools, lifecycle } = boot();
    const session = fakeSession();
    heldRun(session);
    ...
    expect(await send(lifecycle, "@explore check the RPC path")).toEqual({ action: "continue" });
    expect(session.steer).not.toHaveBeenCalled();
  });
  ...
  it("the suggestion popup goes quiet too, so & references stay inactive", async () => {
    const { tools, lifecycle } = boot({ agentMentions: false });
    heldRun(fakeSession());
    const tui = ctx({
      mode: "tui",
    ...
    const factory = vi.mocked(tui.ui.addAutocompleteProvider).mock.calls[0][0] as any;
    const delegated = { items: [{ value: "literal", label: "literal" }], prefix: "&ex" };
    const provider = factory({ getSuggestions: vi.fn().mockResolvedValue(delegated), applyCompletion: vi.fn() });

    expect(await provider.getSuggestions(["&ex"], 0, 3, { signal: new AbortController().signal })).toBe(delegated);
...
// Handles work for the model as well as the human: one namespace, whoever is
