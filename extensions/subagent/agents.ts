...
export interface AgentConfig {
	...
	modelOptions?: { thinking?: ThinkingLevel };
	systemPrompt: string;
	source: "user" | "project";
	filePath: string;
}
...
export function parseModelOptions(value: unknown): { thinking?: ThinkingLevel } | undefined {
	if (value === undefined) return undefined;
	if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("modelOptions must be an object");
	const options = value as Record<string, unknown>;
	const unknown = Object.keys(options).filter((key) => key !== "thinking");
	if (unknown.length > 0) throw new Error(`Unsupported modelOptions: ${unknown.join(", ")}`);
	if (options.thinking === undefined) return undefined;
	const levels = ["off", "minimal", "low", "medium", "high", "xhigh", "max"];
	if (typeof options.thinking !== "string" || !levels.includes(options.thinking)) {
		throw new Error(`modelOptions.thinking must be one of: ${levels.join(", ")}`);
	}
	return { thinking: options.thinking as ThinkingLevel };
}
...
function loadAgentsFromDir(dir: string, source: "user" | "project"): AgentConfig[] {
	...
	for (const entry of entries) {
		...
		agents.push({
			...
			fallbackModels: parseStringList(frontmatter.fallbackModels),
			modelOptions: parseModelOptions(frontmatter.modelOptions),
			systemPrompt: body,
			source,
			filePath,
		});
