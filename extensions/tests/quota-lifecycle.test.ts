...
writeFileSync(
  ...
  JSON.stringify({
    anthropic: { access: "test-token", expires: Date.now() + 3_600_000 },
  }),
);
...
async function harness() {
  ...
  globalThis.clearInterval = ((id: number) =>
    void intervals.delete(id)) as unknown as typeof globalThis.clearInterval;
  ...
  const pi = {
    ...
    events: {
      emit: (_evt: string, value: unknown) => void emitted.push(value),
    },
    registerCommand: () => {},
  };
  ...
  return {
    ...
    settle: () => new Promise((resolve) => realSetTimeout(resolve, 50)),
    restore: () => {
...
test("session_start arms exactly one timer, and it is the fetch timer", async () => {
  ...
  try {
    ...
    assert.equal(
      h.intervals.size,
      1,
      "the 60s countdown repaint timer must be gone",
    );
    assert.ok(
      (h.tick()?.ms ?? 0) >= 60_000,
      "the remaining timer is the periodic fetch, not a repaint",
    );
  } finally {
    h.restore();
  }
...
test("a timer tick on a stale ctx does not throw and tears its timer down", async () => {
  ...
  try {
    ...
    assert.doesNotThrow(
      () => h.tick()?.fn(),
      "stale tick must not escape into the timer callback",
    );
    await h.settle();
...
test("a later session still renders after a stale teardown", async () => {
  ...
  try {
    ...
    assert.equal(
      h.intervals.size,
      1,
      "a fresh session must re-arm the fetch timer",
    );
...
test("quota:changed carries raw windows so the footer can tick the countdown itself", async () => {
  ...
  try {
    await h.fire("session_start");
    const payload = h.emitted.at(-1) as {
      wins: { label: string; resetAt?: number }[];
      showReset: boolean;
    };

    assert.ok(payload, "session_start must emit quota:changed");
    assert.ok(
      Array.isArray(payload.wins),
      "payload must carry raw windows, not pre-formatted text",
    );
    assert.deepEqual(
    ...
    assert.equal(
      typeof payload.showReset,
      "boolean",
      "showReset must ride along for separate module copies",
    );
  } finally {
    h.restore();
  }
