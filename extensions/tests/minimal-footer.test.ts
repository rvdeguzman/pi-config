...
import minimalFooter, {
  formatFooter,
  formatGitSuffix,
} from "../minimal-footer.ts";
...
test("/reset toggle hides countdowns", () => {
  ...
  try {
    assert.equal(
      formatQuota([{ label: "5h", pct: 100, resetAt: now + 3_600_000 }], now),
      "5h 100%",
    );
  } finally {
    setShowReset(true);
...
test("redraws when fast mode changes", () => {
  ...
  const priorityListener = (enabled: unknown) =>
    listeners.get("openai-codex-priority:changed")?.(enabled);
  let sessionStart: ((event: unknown, ctx: unknown) => void) | undefined;
  let footerFactory:
    ((tui: unknown, theme: unknown) => { dispose(): void }) | undefined;
  const pi = {
    events: {
  ...
  sessionStart?.(
    ...
    {
      mode: "tui",
      ui: { setFooter: (factory: never) => (footerFactory = factory) },
    },
  );
...
test("shows fast only when OpenAI Codex fast mode is on", () => {
  ...
  assert.doesNotMatch(
    formatFooter(
      { ...ctx, model: { ...ctx.model, provider: "anthropic" } } as never,
      100,
      true,
    )[0],
    /fast /,
