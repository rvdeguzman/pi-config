...
export function registerAutoDelegation(pi: ExtensionAPI, dependencies: Dependencies): void {
	...
	async function prepare(policy: RoutingPolicy, ctx: ExtensionContext, delivery: "async" | "blocking", allowWrites: boolean, agent?: string): Promise<PreparedCandidate[]> {
		...
		for (const profile of await listProfiles()) {
			...
			for (const candidate of rubric.models) {
				...
				const pinned = scope.get(candidate.id)?.thinkingLevel;
				// Auto-routed effort is chosen by Jev, not inherited from the profile/parent.
				// An explicit scoped pin is a hard constraint: never silently clamp it.
				const thinkingLevels = pinned === undefined ? supported : supported.filter((level) => level === pinned);
				if (thinkingLevels.length) models.push({ ...candidate, thinkingLevels });
			}
	...
	pi.registerTool({
		...
		description: "Opt-in automatic delegation of one parent-authored task. Requires /delegate-auto on. Jev gates dispatch, selects an eligible profile and scoped model/effort pair, then launches via Herdr. Uncertainty/errors return action=parent and launch nothing. Optional agent pins a profile but still evaluates delegation and selects effort. Explicit &name references must use herdr_async instead. No fire-and-forget path.",
		...
		parameters: Type.Object({
			...
			agent: Type.Optional(Type.String({ description: "Optionally pin an existing profile; Jev still gates dispatch and selects its model/effort." })),
		...
		async execute(id, params, signal, onUpdate, ctx) {
			...
			try {
				...
				if (!selection || !selectedModel?.thinkingLevels.includes(decision.thinking) || JSON.stringify(selection) !== JSON.stringify(original)) return retained("Selected model/effort route changed or is no longer eligible.", decision);
				...
				const profile: AgentProfile = { ...selection.profile, model: decision.model, tools: selection.candidate.tools, thinking: decision.thinking };
				...
				return {
					...
					content: [{ type: "text", text: truncateHead(
						`Jev delegated to ${profile.name} on ${decision.model} (${decision.thinking}).\n\n` + result.content
