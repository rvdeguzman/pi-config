...
const isQuotaPayload = (v: unknown): v is QuotaPayload =>
  typeof v === "object" &&
  v !== null &&
  Array.isArray((v as QuotaPayload).wins);
...
function renderQuotaText(quota: QuotaValue): string {
  ...
  if (isQuotaPayload(quota))
    return formatQuota(quota.wins, Date.now(), quota.showReset);
...
export function formatFooter(
  ...
  const cwd =
    (ctx.cwd === home
      ? "~"
      : ctx.cwd.startsWith(home + sep)
        ? `~${ctx.cwd.slice(home.length)}`
        : ctx.cwd) + formatGitSuffix(ctx.cwd, gitBranch);
  ...
  const fast =
    model?.provider === "openai-codex" && priorityEnabled ? " fast" : "";
  ...
  const shownCwd = truncateToWidth(
    cwd,
    Math.max(0, width - visibleWidth(shownRight) - 2),
    "",
  );
  const topPadding = " ".repeat(
    Math.max(0, width - visibleWidth(shownCwd) - visibleWidth(shownRight)),
  );
  const shownLeft = truncateToWidth(left, width, "");
  const quotaText = renderQuotaText(quota);
  const shownQuota = truncateToWidth(
    quotaText,
    Math.max(0, width - visibleWidth(shownLeft) - (quotaText ? 2 : 0)),
    "",
  );
  const padding = shownQuota
    ? " ".repeat(
        Math.max(0, width - visibleWidth(shownLeft) - visibleWidth(shownQuota)),
      )
    : "";
